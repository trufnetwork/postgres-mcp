...
class GeneralStreamTool:
    ...
    async def get_index_change(self, current_data: List[Dict[str, Any]], prev_data: List[Dict[str, Any]], time_interval: int) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_index_changes(
        self, current_data: List[Dict[str, Any]], prev_data: List[Dict[str, Any]], time_interval: int
    ) -> List[Dict[str, Any]]:
        ...
        for current_record in current_sorted:
            ...
            while j + 1 < len(prev_sorted) and int(prev_sorted[j + 1]["event_time"]) <= target_time:
                j += 1

            # Check if we found a valid previous value
            if j < len(prev_sorted) and int(prev_sorted[j]["event_time"]) <= target_time:
                ...
                if prev_value != 0:
                    ...
                    changes.append({"event_time": current_time, "value": str(change_percent)})
