...
from postgres_mcp.truf.query import PRIMITIVE_STREAM_LAST_RECORD_QUERY
from postgres_mcp.truf.query import PRIMITIVE_STREAM_RECORD_QUERY
from postgres_mcp.truf.query import STREAM_REF_QUERY
...
class PrimitiveStreamTool:
    ...
    async def get_index(
        ...
        try:
            ...
            if effective_base_time is None:
                ...
                metadata_rows = await SafeSqlDriver.execute_param_query(self.sql_driver, metadata_query, [stream_ref])

                if metadata_rows:
                    effective_base_time = metadata_rows[0].cells.get("value_i")

            # Get base value
            base_value = await self._get_base_value(data_provider, stream_id, effective_base_time, frozen_at)
            ...
            if from_time is None and to_time is None:
                latest_record = await self._get_last_record_primitive(data_provider, stream_id, None, effective_frozen_at)
            ...
            records = await self._get_record_primitive(data_provider, stream_id, from_time, to_time, frozen_at)
    ...
    async def _get_stream_ref(self, data_provider: str, stream_id: str) -> int:
        ...
        rows = await SafeSqlDriver.execute_param_query(self.sql_driver, STREAM_REF_QUERY, [data_provider, stream_id])
    ...
    async def _get_record_primitive(
        self, data_provider: str, stream_id: str, from_time: Optional[int], to_time: Optional[int], frozen_at: Optional[int]
    ) -> List[Dict[str, Any]]:
        max_int8 = 9223372036854775000
        ...
        params = [
            ...
            effective_frozen_at,
            effective_from,
            effective_to,  # interval_records
            stream_ref,
            effective_from,
            effective_frozen_at,  # anchor_record
        ]

        rows = await SafeSqlDriver.execute_param_query(self.sql_driver, PRIMITIVE_STREAM_RECORD_QUERY, params)
        ...
        records = []
        for row in rows:
            records.append({"event_time": row.cells.get("event_time"), "value": str(row.cells.get("value", "0"))})

        return records

    async def _get_last_record_primitive(self, data_provider: str, stream_id: str, before: Optional[int], frozen_at: int) -> Optional[Dict[str, Any]]:
        max_int8 = 9223372036854775000
        effective_before = before if before is not None else max_int8

        stream_ref = await self._get_stream_ref(data_provider, stream_id)

        rows = await SafeSqlDriver.execute_param_query(self.sql_driver, PRIMITIVE_STREAM_LAST_RECORD_QUERY, [stream_ref, effective_before, frozen_at])

        if rows:
            return {"event_time": rows[0].cells.get("event_time"), "value": str(rows[0].cells.get("value", "0"))}

        return None

    async def _get_base_value(self, data_provider: str, stream_id: str, base_time: Optional[int], frozen_at: Optional[int]) -> Decimal:
        effective_base_time = base_time
        stream_ref = await self._get_stream_ref(data_provider, stream_id)
        ...
        if effective_base_time is None:
            ...
            if effective_base_time is None:
                first_record = await self._get_first_record_primitive(data_provider, stream_id, frozen_at)
                if first_record:
        ...
        exact_records = await self._get_record_primitive(data_provider, stream_id, effective_base_time, effective_base_time, frozen_at)
        ...
        before_record = await self._get_last_record_primitive(
            data_provider, stream_id, effective_base_time, frozen_at if frozen_at is not None else 9223372036854775000
        )
        ...
        after_record = await self._get_first_record_primitive(data_provider, stream_id, frozen_at, effective_base_time)
    ...
    async def _get_first_record_primitive(
        self, data_provider: str, stream_id: str, frozen_at: Optional[int], after_time: Optional[int] = None
        ...
        rows = await SafeSqlDriver.execute_param_query(self.sql_driver, query, params)
