  ...
  WITH RECURSIVE
            ...
            query_vars AS (
                SELECT 
                    ...
                    COALESCE({}, 9223372036854775000) as effective_to,
                    COALESCE({}, 9223372036854775000) as effective_frozen_at,
                    COALESCE({}, 0) as effective_base_time  -- TODO: Add metadata lookup logic
            ),
            ...
            -- Latest mode check and override logic
            latest_mode_calc AS (
                SELECT 
                    src.*,
                    (src.from_param IS NULL AND src.to_param IS NULL) as is_latest_mode
            ...
            -- Get latest event time if in latest mode
            latest_event_time AS (
                SELECT 
                    -- TODO: This should call get_last_record_composed equivalent
                    EXTRACT(EPOCH FROM NOW())::bigint as latest_time
                WHERE EXISTS (SELECT 1 FROM latest_mode_calc WHERE is_latest_mode = true)
            ),
            
            -- Override effective_from and effective_to for latest mode
            effective_bounds AS (
                SELECT 
                    CASE 
                        WHEN lmc.is_latest_mode AND let.latest_time IS NOT NULL 
                        THEN let.latest_time 
                        ELSE lmc.effective_from 
                    END as final_from,
                    CASE 
                        WHEN lmc.is_latest_mode AND let.latest_time IS NOT NULL 
                        THEN let.latest_time 
                        ELSE lmc.effective_to 
                    END as final_to,
                    lmc.*
                FROM latest_mode_calc lmc
                LEFT JOIN latest_event_time let ON 1=1
            ),
            ...
            taxonomy_true_segments AS (
                SELECT
                    ...
                    t.weight_for_segment,
                    t.segment_start,
                    COALESCE(pns.next_start_time, eb.max_int8) - 1 AS segment_end
                FROM (
                    SELECT
                        tx.stream_ref,
                        tx.child_stream_ref,
                ...
                JOIN parent_next_starts pns
                  ON t.stream_ref = pns.stream_ref
                 AND t.segment_start = pns.start_time
                CROSS JOIN effective_bounds eb
            ),

            -- Hierarchy with effective weight calculation (key difference from get_record_composed)
            hierarchy AS (
                -- Base case
                SELECT
                    tts.stream_ref AS root_stream_ref,
                ...
                FROM taxonomy_true_segments tts
                CROSS JOIN effective_bounds eb
                WHERE tts.stream_ref = eb.stream_ref
                  AND tts.segment_end >= COALESCE(
                      (SELECT t_anchor_base.start_time FROM main.taxonomies t_anchor_base
                       WHERE t_anchor_base.stream_ref = eb.stream_ref
                         AND t_anchor_base.disabled_at IS NULL 
                         AND t_anchor_base.start_time <= eb.final_from
                       ORDER BY t_anchor_base.start_time DESC, t_anchor_base.group_sequence DESC 
                       LIMIT 1),
                      0
                  )
                  AND tts.segment_start <= eb.final_to
                ...
                WHERE GREATEST(h.path_start, tts.segment_start) <= LEAST(h.path_end, tts.segment_end)
                  ...
                  AND GREATEST(h.path_start, tts.segment_start) <= eb.final_to
                  AND h.level < 10
            ),
            ...
            -- Primitive weights (pass-through)
            primitive_weights AS (
                SELECT
                    hpp.primitive_stream_ref,
                    hpp.raw_weight,
            ...
            cleaned_event_times AS (
                ...
                FROM (
                    ...
                    CROSS JOIN effective_bounds eb
                    WHERE pe.event_time > eb.final_from
                      AND pe.event_time <= eb.final_to
                      AND pe.created_at <= eb.effective_frozen_at
                    ...
                    FROM primitive_weights pw
                    CROSS JOIN effective_bounds eb
                    WHERE pw.group_sequence_start > eb.final_from
                      AND pw.group_sequence_start <= eb.final_to
                ) all_times_in_range
            ...
            initial_primitive_states AS (
                ...
                FROM (
                    ...
                    FROM main.primitive_events pe_inner
                    CROSS JOIN effective_bounds eb
                    WHERE pe_inner.event_time <= eb.final_from
                      AND EXISTS (
                          SELECT 1 FROM primitive_weights pw_exists
                          WHERE pw_exists.primitive_stream_ref = pe_inner.stream_ref
                      )
                      AND pe_inner.created_at <= eb.effective_frozen_at
                ) pe
                WHERE pe.rn = 1
            ),
            ...
            primitive_events_in_interval AS (
                ...
                FROM (
                    ...
                    CROSS JOIN effective_bounds eb
                    WHERE pe_inner.event_time > eb.final_from
                        AND pe_inner.event_time <= eb.final_to
                        AND pe_inner.created_at <= eb.effective_frozen_at
                ) pe
                WHERE pe.rn = 1
            ),
            ...
            -- Base value calculation (key difference for index calculation)
                    ...
                    COALESCE(
                        ...
                        (SELECT pe.value FROM main.primitive_events pe
                         ...
                         WHERE pe.stream_ref = dp.stream_ref
                           AND pe.event_time = eb.effective_base_time
                           AND pe.created_at <= eb.effective_frozen_at
            ...
            effective_weight_changes AS (
                ...
                WHERE GREATEST(pw.group_sequence_start, fvt.first_value_time) <= pw.group_sequence_end
                  AND pw.raw_weight != 0::numeric(36,18)
                  AND pw.group_sequence_end < (eb.max_int8 - 1)
            ),
            ...
            unified_events AS (
                SELECT
                    pec.stream_ref,
                    pec.event_time,
                    pec.delta_indexed_value, -- Use indexed values
                    0::numeric(36,18) AS weight_delta,
                    1 AS event_type_priority
                FROM primitive_event_changes pec
            ...
            anchor_time_calc AS (
                ...
                CROSS JOIN effective_bounds eb
                WHERE time_point < eb.final_from
            ),
            ...
            filtered_mapping AS (
                ...
                CROSS JOIN effective_bounds eb
                WHERE (fm.event_time >= eb.final_from AND fm.event_time <= eb.final_to)
            ...
            range_check AS (
                SELECT EXISTS (
                    ...
                    CROSS JOIN effective_bounds eb
                    WHERE fm_check.event_time >= eb.final_from
                      AND fm_check.event_time <= eb.final_to
            ...
            direct_hits AS (
                ...
                CROSS JOIN effective_bounds eb
                WHERE la.event_time >= eb.final_from
                  AND la.event_time <= eb.final_to
            ...
            anchor_hit AS (
                ...
                WHERE la.anchor_time IS NOT NULL
                  ...
                  AND eb.final_from > la.anchor_time
                  AND la.final_event_time IS NOT NULL
                  AND NOT EXISTS (
                      SELECT 1 FROM locf_applied dh
                      CROSS JOIN effective_bounds eb2
                      WHERE dh.event_time = eb2.final_from
                  )
            ),
