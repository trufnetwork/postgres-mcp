...
class ComposedStreamTool:
    ...
    async def get_record_composed(
        ...
        try:
            ...
            # Execute the query
            rows = await SafeSqlDriver.execute_param_query(self.sql_driver, COMPOSED_STREAM_RECORD_QUERY, params)
            ...
            records = []
            for row in rows:
                record = {"event_time": row.cells.get("event_time"), "value": str(row.cells.get("value", "0"))}
    ...
    async def describe_taxonomies(self, data_provider: str, stream_id: str, latest_group_sequence: bool = True) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            # Execute the query
            rows = await SafeSqlDriver.execute_param_query(self.sql_driver, TAXONOMIES_QUERY, params)
    ...
    async def get_index(
        ...
        try:
            ...
            rows = await SafeSqlDriver.execute_param_query(self.sql_driver, COMPOSED_STREAM_INDEX_QUERY, params)
